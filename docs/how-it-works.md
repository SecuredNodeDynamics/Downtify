...
When *Organize by artist* is enabled, the file is placed inside a subfolder named after the primary artist. With *Organize by album* also enabled, files go into `Artist/Album/` instead. Otherwise, single tracks go into the root of the downloads directory and playlist/album tracks go into a per-playlist or per-album subfolder.
