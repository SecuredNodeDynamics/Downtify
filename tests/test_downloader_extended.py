...
def test_album_subdir_returns_album_name():
    result = Downloader._album_subdir({'album_name': 'AM'})
    assert result == 'AM'


def test_album_subdir_missing_album_returns_unknown():
    assert Downloader._album_subdir({}) == 'Unknown Album'
    assert Downloader._album_subdir({'album_name': ''}) == 'Unknown Album'


def test_album_subdir_sanitizes_slashes():
    result = Downloader._album_subdir({'album_name': 'Live/At Wembley'})
    assert '/' not in result
...
def test_organize_by_album_finds_file_in_artist_album_dir(tmp_path):
    d = _make(tmp_path, organize_by_artist=True, organize_by_album=True)
    album_dir = tmp_path / 'Arctic Monkeys' / 'AM'
    album_dir.mkdir(parents=True)
    (album_dir / 'Arctic Monkeys - Do I Wanna Know.mp3').write_bytes(b'\x00')
    result = d.existing_filename_for({
        'name': 'Do I Wanna Know',
        'artists': ['Arctic Monkeys'],
        'album_name': 'AM',
    ...
    assert (
        result == 'Arctic Monkeys/AM/Arctic Monkeys - Do I Wanna Know.mp3'
    )


def test_organize_by_album_ignores_artist_only_path(tmp_path):
    d = _make(tmp_path, organize_by_artist=True, organize_by_album=True)
    artist_dir = tmp_path / 'Artist'
    artist_dir.mkdir()
    (artist_dir / 'Artist - Song.mp3').write_bytes(b'\x00')
...
def test_organize_by_album_without_artist_uses_playlist_subdir(tmp_path):
    d = _make(tmp_path, organize_by_artist=False, organize_by_album=True)
    pl_dir = tmp_path / 'My Playlist'
    pl_dir.mkdir()
    (pl_dir / 'Artist - Song.mp3').write_bytes(b'\x00')
    result = d.existing_filename_for(
...
def test_organize_by_album_default_is_false(tmp_path):
    d = Downloader(tmp_path)
    assert d.organize_by_album is False
